...
//		add_action('profile_update', array(&$this, 'avatar_upload'));												// Perform avatar image upload.
	...
	function manage_avatar_cache()
	{
		...
		<div class="wrap">
			...
			<b><?php _e('User Avatars', 'avatars'); ?></b>
			<?php
				...
				if(!get_option('show_avatars')) {
					...
					<?php
						...
						if($all) {
							...
							if($coms) { ?>
								<b><?php _e('Commenter Avatars', 'avatars'); ?></b>
			...
			<b><?php _e('Avatar Options', 'avatars'); ?></b>
			...
			<b><?php _e('Avatars Usage', 'avatars'); ?></b>
	...
	{ ?>
		...
		<?php printf("<b>%s</b>", __('Avatar', 'avatars')); ?>
	...
	function avatars_manage()
	{
		...
		if(($uid == $current_user->ID || current_user_can('edit_users')) &&  is_numeric($_GET['uid'])) { ?>
			...
			<body>
				...
				<span style='width: 150px; float: left; padding:2px;'>
					<b>Current Avatar</b>
					<?php
						printf("<p><small>%s</small></p>", __('Imagem do avatar atual', 'avatars'));
						...
						if($avatar_type == TYPE_LOCAL) {
							printf("<p><a id='avatars_local_button' class='button' href='%s?action=avatars_manage&act=DEL&uid=%s&n=%s'>%s</a></p>",
								...
								$uid,
								$nonce,
								__('Delete para usar uma imagem nova', 'avatars')
							);

							list($w, $h, $type, $attr) = getimagesize($root . $files[BASE_FILE]);

							$img_insert = sprintf("<div style='width: 489px; height: 200px;'><img id='avatar_upload' src='%s' /></div>", $files[BASE_FILE]);
						}
					?>
					...
					<b style='margin-top: 80px;'>Novo Avatar</b>
					<?php
						printf("<p><small>%s</small></p>", __('Como a imagem de upload vai ficar depois de cortada.', 'avatars'));
				...
				<span style='width: 497px; padding:2px; float: right;'>
					<b>Avatar Upload</b>
					<?php 

					printf("<p><small>%s</small></p>", __('Faça upload de uma imagem para usar como Avatar. Imagens quadradas com 20k é o ideial', 'avatars')); ?>
					<input type='file' name='avatar_file' id='avatar_file' style='width: 486px;' />
					<?php
						printf("<p>%s</p>", $img_insert);
						printf("<p><input type='submit' value='Subir e atualizar seu avatar' class='btn btn-primary pull-right' /></p>");
					?>
